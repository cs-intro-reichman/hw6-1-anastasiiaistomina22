...
public class Runigram {
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
        ...
        double factorW = (double)image[0].length / width;
		double factorH = (double)image.length / height;
		for(int i = 0; i < height; i++){
			for(int j = 0; j < width; j++){
                imageNew[i][j] = image[(int)(i * factorH)][(int)(j * factorW)];
			}
		}
		return imageNew;
	}
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		Color[][] targetNew = scaled(target, source[0].length,source.length );
          
        for(int i = 0; i <= n; i++){
            double alpha = (n - i) / (double)n;

			Color[][] intermediate = blend(source, targetNew, alpha);
